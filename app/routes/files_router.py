    ...
    for file in files:
        ...
        try:
            ...
            try:
                ...
                logger.error(f"Database error creating document record for {file.filename}: {e}")
                results.append({
                    "filename": file.filename,
                    "status": "failed",
            ...
            # Update document metadata with pages and chunks count
            ...
            for result in results:
                if result["status"] == "success":
                    try:
                        ...
                        logger.error(f"Database error marking {result['filename']} as completed: {db_error}")
                    except Exception as db_error:
                        logger.error(f"Unexpected error marking {result['filename']} as completed: {db_error}")
